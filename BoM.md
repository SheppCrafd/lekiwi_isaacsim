...
Built the same way as the lidar mount above: a real STL generated against the base plate's actual measured hole grid (`urdf/meshes/camera_base_base_plate_layer1_v5.stl`, cross-sectioned directly, not assumed) and, as of a 2026-08-11 correction, the actual **B0200** SKU's own published mechanical spec (Arducam's B0200 Quick Start Guide, real 38×38mm board / 34×34mm square mount pitch) rather than a borrowed sibling SKU's datasheet — see `isaac_sim/README_lekiwi_variants.md`'s camera-mount section for the full sourcing and the earlier wrong-sibling-SKU mistake this replaced.

**Also two screw sizes, same reasoning as the lidar mount:**
- **M3** — 5 screws, base flange to the real base-plate holes (measured ~1.727mm hole radius on the actual plate mesh, i.e. a real M3 clearance hole, not picked to match an assumed screw size). Corrected from an earlier "6" (2026-08-11): the flange's real frame shape only uses 5 of the 6 base-plate holes originally extracted — the 6th would've needed solid material where the frame was deliberately routed around `servo_controller_mount_v3` instead.
- **M2.5** — 4 screws, Arducam board to the bracket's riser face, now on a real **34×34mm square** pattern sourced directly from Arducam's own B0200 Quick Start Guide (2026-08-11, corrected from a borrowed-sibling-SKU 16×28mm rectangle — see `isaac_sim/README_lekiwi_variants.md`). Hole diameter still **is** a picked default — Arducam doesn't publish one for either of B0200's two real hole-pitch options, so M2.5 clearance was chosen to match the hardware already in this build (see Lidar Mount Hardware above), not measured from a drawing.

**Likely coverable by the same combined insert kit purchased for the lidar mount** — that kit only needed 4×M3 + 2×M2.5; a "~10-20pc" kit has headroom for this mount's 5×M3 + 4×M2.5 on top, meaning the two mounts probably don't need two separate kit purchases. Confirm total counts (9×M3 + 6×M2.5 across both mounts) against whatever specific kit size you actually buy.

| Component | Qty needed | Price (USD) | Notes |
|-----------|-----------|-----------|------|
| **M3 x 8mm socket head cap screws** (5 needed — bracket base flange, 2mm thick, so a shorter screw than the lidar mount's M3x12) | 5 | ~$8–12 | ⚠️ Price not confirmable via automated fetch — check at checkout; likely already covered by the assorted-length kit bought for the lidar mount |
